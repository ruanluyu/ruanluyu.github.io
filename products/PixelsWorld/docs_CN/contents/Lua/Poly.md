...
- `V`有一维、二维、三维、四维、字符串、贴图id和索引组共7种。
- 对于任意浮点数`x,y,z,w`
- `x`或`{x}`称作一维`V`。
- `{x,y}`称作二维`V`。
- `{x,y,z}`称作三维`V`。
...
- (`K`的末尾四个字符`"_tex"`)且(`V`是整数且对应贴图存在)时，称作贴图id`V`。
...
> - 当某整数属性值的键名称后缀带有`_tex`时，该属性将被视为贴图id. 
> - 使用自定义shader可以同时在一个prim中使用多个贴图。
> - [更多关于贴图的信息](./Texture.md)
